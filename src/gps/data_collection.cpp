...
double lon, lat, last_lon, last_lat;	//存储经纬度数据	
...
void gpsCallback(const sensor_msgs::NavSatFix& gps)
{
	...
	if (gps_flag == 0){
		...
		printf("last lon %lf, last lat %lf \n", last_lon, last_lat);
		string file_path;
		file_path = getenv("HOME") + string("/gps_ws/src/mygps/info/ggm");	
		ofstream writefile;
		writefile.open(file_path, ios::app);
	...
	writefile << to_string(gps.longitude )<< "," << to_string(gps.latitude) << endl;
	writefile.close();
	...
	// lat=last_lat;
	printf("last lon %lf, last lat %lf \n", last_lon, last_lat);
	x = zoom*get_distance(last_lon, last_lat, origin_lon, origin_lat)*cos(get_degree(last_lon, last_lat, origin_lon, origin_lat));
	y = zoom*get_distance(last_lon, last_lat, origin_lon, origin_lat)*sin(get_degree(last_lon, last_lat, origin_lon, origin_lat));
...
int main( int argc, char** argv )
{
	...
	ros::NodeHandle n;
	ros::Rate r(1);
	gps_sub = n.subscribe("/gps/fix",10,gpsCallback);	//订阅gps话题
...
*/
	...
	*/
	if(1){
		printf("东北方向%f\n",get_degree(107,30,106,29)/(PI / 180));
		printf("东南方向%f\n",get_degree(107,28,106,29)/(PI / 180));
		printf("西南方向%f\n",get_degree(106,28,106,29)/(PI / 180));
		printf("西北方向%f\n",get_degree(106,30,106,29)/(PI / 180));
	}
	//------获取方位角数据测试--end----------
	//--- 文件读取与参数初始化 end ----
	...
	while(ros::ok()){
		...
		for (int i = 0; i < 3; i++)
		{
			...
			pose_marker.scale.x=1 * zoom/10;
			pose_marker.scale.y=2 * zoom/10;
			pose_marker.scale.z=1 * zoom/10;
