...
# TODO: the_location的值改为固定经纬度（转换后的坐标）
the_location = []
yawdata = 0
net_state=False  # 网络状态
ttmcp = 0   #计次
msg = myGPS()
...
def DegDis():
    ...
    msg.compass=yawdata
    if net_state == True:   # 固定短距离目标，联网更新位置
        #使用 , 分割元素
        # print(the_location)
    ...
    elif net_state == False:    #发送固定距离和角度
       # global yawdata
